...
public class Login_stepDefs {
    LoginPage loginPage = new LoginPage();        // obje oluşturduk.
    ...
    public void user_should_be_on_the_login_page() {
        Driver.get().get(ConfigurationReader.get("url"));       //sayfaya gidebilmek için url yi tanımladık.
    }

    @When("User should input login information")
    public void user_should_input_login_information() {
        loginPage.loginUser();                            // sayfaya ulaşmak için en yukarıda obje oluşturduk.
    }
    ...
    public void the_warning_message_contains(String warning) {

        BrowserUtils.waitFor(1);
        String actualWarningMessage = loginPage.warning(warning);
        Assert.assertEquals(warning,actualWarningMessage);
    }
